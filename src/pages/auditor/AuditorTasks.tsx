...
const AuditorTasks: React.FC = () => {
  ...
  const handleStartVisit = (visitId: string) => {
    toast({
      title: "Visit started",
      description: "Navigating to live visit checklist",
    });
    navigate(`/auditor/live-visit-checklist?id=${visitId}`);
  };
  
  const handleViewAuditForm = (cspId: string) => {
    navigate(`/auditor/audit-form?csp=${cspId}`);
  };
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => handleStartVisit(row.id)}>
            Start Visit
          </Button>
          <Button variant="ghost" size="sm" onClick={() => handleViewAuditForm(row.csp_id)}>
            Form
          </Button>
        </div>
      )
    ...
    <div className="container mx-auto p-4 space-y-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="default" onClick={() => navigate('/auditor/visit-logs')}>
            <Calendar className="mr-2 h-4 w-4" /> View All Visits
          </Button>
        </div>
      </div>
        ...
        <Card className="border-l-4 border-l-purple-500">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">
              {tasks.reduce((sum, task) => sum + task.distance, 0).toFixed(1)} km
            </div>
      ...
      {/* Red Zone Alert */}
      {tasks.filter(task => task.red_zone).length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
          <AlertTriangle className="h-5 w-5 text-red-500 mt-0.5" />
          <div>
            <p className="font-medium text-red-800">Red Zone Alert</p>
            <p className="text-red-700 text-sm">
              {tasks.filter(task => task.red_zone).length} audit locations are in designated red zones. 
              Use the Red Zone Protocol for these visits.
            </p>
            <Button 
              variant="outline" 
